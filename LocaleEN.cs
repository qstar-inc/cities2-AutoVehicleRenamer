    ...
    public class LocaleEN : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.Separator)), "Choose a separator" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.Separator)), $"Type what you want as the character(s) you want as the separator among the vehicle name and origin name. Using emoji or other unsupported characters might crash the game. To be safe, save first, then rename a vehicle manually, let the simulation run for a few seconds, save the game again on a separate file, exit the game, start the game, load the save, unpause the simulation, if this doesn't crash the game, you're good to go." },

                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.TextFormat)), "Choose a text format" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.TextFormat)), $"Select one of the text formatting to use on the renaming script." },
                { m_Setting.GetEnumValueLocaleID(Setting.TextFormatEnum.Value1), "{vehicle} {separator} {building}" },
                { m_Setting.GetEnumValueLocaleID(Setting.TextFormatEnum.Value2), "{building} {separator} {vehicle}" },

                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.RestoreDefaults)), "Restore Defaults" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.RestoreDefaults)), $"Disables renaming vehicles from buildings with default names, sets the separator to \"•\", and sets the format to \"{{vehicle}} {{separator}} {{building}}\"" },
                { m_Setting.GetOptionWarningLocaleID(nameof(Setting.RestoreDefaults)), "Confirm restore defaults settings\nfor 'Auto Vehicle Renamer'?" },
